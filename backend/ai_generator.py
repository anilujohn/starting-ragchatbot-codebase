...
"""
    ...
    def generate_response_with_rounds(
        self,
        query: str,
        ...
        state = ConversationState(
            ...
            base_params={**self.base_params, "system": system_content},
        )

        # Execute rounds until termination condition met
        while state.round_count < state.max_rounds:
            response, should_continue, termination_reason = self._execute_single_round(
                state
            )
        ...
        final_params = {**state.base_params, "messages": state.messages}
    ...
    def _execute_single_round(
        ...
        api_params = {**state.base_params, "messages": state.messages}
    ...
    def _should_continue_rounds(
        ...
        if (
            state.round_count >= state.max_rounds - 1
        ):  # -1 because we increment after this check
