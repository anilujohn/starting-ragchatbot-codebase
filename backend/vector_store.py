...
class SearchResults:
    ...
    lesson_links: Optional[Dict[str, str]] = (
        None  # Maps "course_title|lesson_number" to lesson_link
    )
...
class VectorStore:
    ...
    def search(
        ...
        try:
            ...
            if not search_results.is_empty():
                search_results.lesson_links = self.get_lesson_links_for_results(
                    search_results
                )
    ...
    def get_lesson_links_for_results(
        ...
        for course_title in courses_needed:
            try:
                ...
                catalog_results = self.course_catalog.get(
                    ids=[course_title], include=["metadatas"]
                )

                if catalog_results["metadatas"]:
                    lessons_json = catalog_results["metadatas"][0].get(
                        "lessons_json", "[]"
                    )
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict[str, Any]]:
        ...
        Args:
            course_name: Course name/title (partial matches work)
        ...
        try:
            ...
            # Get course metadata from catalog
            results = self.course_catalog.get(ids=[course_title], include=["metadatas"])
