...
class TestCourseSearchTool(unittest.TestCase):
    ...
    def test_execute_with_course_and_lesson_filter(self):
        ...
        result = self.search_tool.execute(
            "specific query", course_name="Target Course", lesson_number=5
        )

        self.mock_vector_store.search.assert_called_once_with(
            query="specific query", course_name="Target Course", lesson_number=5
        )
    ...
    def test_sources_tracking(self):
        ...
        mock_results = SearchResults(
            ...
            distances=[0.3, 0.4],
            lesson_links={
                "Course A|1": "http://link1.com",
                "Course B|2": "http://link2.com",
            },
        )
        self.mock_vector_store.search.return_value = mock_results
