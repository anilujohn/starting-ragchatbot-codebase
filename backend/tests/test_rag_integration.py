...
class TestRAGIntegration(unittest.TestCase):
    ...
    def test_rag_system_initialization(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_query_without_session(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
        ...
        mock_tool_manager.get_last_sources.return_value = [{"text": "MCP Course - Lesson 1", "link": "http://test.com"}]
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_query_with_session(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
        """Test query processing with session context"""
        # Mock session manager
        mock_session_instance = Mock()
        mock_session_instance.get_conversation_history.return_value = "Previous conversation history"
        ...
        call_args = mock_ai_instance.generate_response.call_args[1]
        self.assertEqual(call_args["conversation_history"], "Previous conversation history")
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_tool_manager_integration(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
    ...
    @patch('rag_system.DocumentProcessor') 
    @patch('rag_system.SessionManager')
    def test_sources_handling(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_error_handling_in_query(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_get_course_analytics(self, mock_session, mock_doc_proc, mock_vector_store, mock_ai_gen):
        ...
        mock_vector_instance.get_existing_course_titles.return_value = [
            "MCP Course", "Retrieval Course", "Compression Course", "Computer Use Course"
        ]
        ...
        expected = {
            "total_courses": 4,
            "course_titles": ["MCP Course", "Retrieval Course", "Compression Course", "Computer Use Course"]
        }
...
class TestRAGSystemRealConfiguration(unittest.TestCase):
    ...
    def test_config_anthropic_key_present(self):
        ...
        self.assertTrue(config.ANTHROPIC_API_KEY, 
                       "ANTHROPIC_API_KEY is not configured")
        self.assertNotEqual(config.ANTHROPIC_API_KEY, "", 
